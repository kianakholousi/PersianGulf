...
void col_ang()
{
  ...
  if (blocks)
  {
    ...
    {
      if (pcam.blocks[j].signature == 1)//Ball
      {
        BC = 0;
        Bx = pcam.blocks[j].x -170 + (pcam.blocks[j].height / 2);
        Bx = -Bx;
        By = pcam.blocks[j].y - 100 + (pcam.blocks[j].width / 2);
